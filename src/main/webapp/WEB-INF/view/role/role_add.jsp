...
<body class="hold-transition skin-blue sidebar-mini">
  ...
  <aside class="main-sidebar">
    ...
    <div id="main" style="margin: 60px;padding: 50px;background: #AAFFFF">            
            <!--保存操作后的提示信息：成功或者失败-->
            <div id="save_result_info" class="save_success" style="display: none">${emp}</div><!--保存失败，角色名称重复！-->
            <form action="${emp}/role/addRole" id="dForm" method="post" class="main_form">
                    ...
                    <div class="input_info_scroll">
                        <ul>
                           <c:forEach items="${powers}" var="p">	                          
                            	<li><input type="checkbox" name="powername" value="${p.pid}" 
                            	     <c:if test="${p.pid == 1}">
                            	              checked="checked";
                            	     </c:if>
                            	     <c:if test="${p.pid == 9}">
                ...
                <div class="button_info clearfix">
                    <input type="button" value="保存" class="btn_save" id="dsubmit" />
                    <input type="button" value="取消" class="btn_save" />
                </div>
                <input type="hidden" name="adminname" value="${admin1.aname}"/>
            </form>
        </div> 
